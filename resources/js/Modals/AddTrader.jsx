...
function AddTrader({ closeModal, getTradersAgain }) {
    ...
    const postData = async () => {
        ...
        try {
            ...
            console.log(er.response?.data?.errors?.national_id[0]);
            const phoneEr = er.response?.data?.errors?.hasOwnProperty("phone");
            const nationalidEr =
                er.response?.data?.errors?.hasOwnProperty("national_id");
            setSuccessMsg(
                `${phoneEr == true && er.response?.data?.errors?.phone[0]} 
                -- ${
                    nationalidEr == true &&
                    er.response?.data?.errors?.national_id[0]
                }`
            );
            setTimeout(() => {
                setSuccessMsg("");
                ...
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-28">
                    ...
                    <div className="p-6 space-y-6">
                        <form className="flex flex-col items-center gap-3 mt-6 dark:text-black relative">
                            {successMsg.length > 0 && (
                                <div className="msg fixed text-center z-50 top-20 left-0 w-full rounded-md p-2 bg-green-400">
                                    {successMsg}
                                </div>
                            )}
