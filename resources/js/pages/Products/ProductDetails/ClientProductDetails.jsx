...
const ClientProductDetails = () => {
    ...
    useEffect(() => {
        ...
        const getSubCategoriesWithAuth = async () => {
            try {
                const res = await axios.get(
                    `http://127.0.0.1:8000/api/items/${id}`,
                    {
        ...
        if (getToken) {
            let isClient = async () => {
                try {
                    let clientRes = await axios.get(
                        `http://127.0.0.1:8000/api/clients/client`,
                        {
                            headers: {
                                Authorization: `Bearer ${getToken}`,
                            },
    ...
    const whatColor = (colorid) => {
        ...
        console.log(colorid.target.value);
    };
            ...
            <div className="prodcut-details-container p-1 pb-24">
                ...
                >
                    ...
                    >
                        <img
                            // className="w-full"
                            src={`http://127.0.0.1:8000/assets/images/uploads/traders/${singleProduct.traderLogo}`}
                            alt="لا يوجد صورة"
                        />
                    </div>
                    <h1> اسم التاجر: {singleProduct.traderName}</h1>
                </div>
                {/* معلومات التاجر */}
            </div>
